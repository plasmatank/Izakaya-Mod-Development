...
{
    [BepInPlugin("Plasmatank.CustomRecipe", "CustomRecipe", "1.0.0")]
    public class Plugin : BasePlugin
    {
        ...
        static public List<int> Conflicted;

        static public List<int> Installed;

        static public Dictionary<int, GameData.Core.Collections.Recipe> Origin_Recipes;
        ...
        public override void Load()
        {
            ...
            Conflicted = new List<int>{ }; Installed = new List<int>{ };
            Harmony.PatchAll();
        ...
        public static class AwakeLoadHook
        {
            public static bool Prefix()
            {
                ...
                EndHook.Cleanup();
                Conflicted = new List<int> { }; Installed = new List<int> { };
                Origin_Recipes = GameData.Core.Collections.DataBaseCore.Recipes;
                Add_Recipe();                
                if (true && Conflicted.Count > 0)
                {                   
                    Alert();
                }
                return true;
            ...
            public static void Add_Recipe()
            {
                foreach (FileInfo file in new DirectoryInfo(Path.Combine(System.Environment.CurrentDirectory, @"BepInEx\plugins\CustomRecipe")).GetFiles())
                {
                    if (file.Name.EndsWith(".json"))
                    {
                        ...
                        {
                            if (keypair.value.ToString() == recipe.ID + "PLASMATANK")
                            {
                                Conflicted.Add(recipe.ID);
                                Passed = false;
                                Print("Conflicted recipe detected: " + recipe.ID.ToString());
                                break;
                            }
                        ...
                        {   
                            if (!recipe.ReplaceMode)
                            {
                                ...
                                GameData.Core.Collections.DataBaseCore.RecipesMapping.Add(recipe.ID, recipe.ID.ToString() + "PLASMATANK");
                            ...
                            {
                                if (Origin_Recipes.ContainsKey(recipe.ID))
                                {
        ...
        [HarmonyPatch(typeof(ResultScene.SceneManager), nameof(ResultScene.SceneManager.GotoLoadScene))]
        public static class EndHook
        {
            public static bool Prefix()
            {
                Print("Good Night! :D");
            ...
            public static void Cleanup()
            {
                if (Installed.Count > 0)
                {
                    ...
                    {
                        GameData.CoreLanguage.Collections.DataBaseLanguage.Foods.Remove(i);
                        GameData.CoreLanguage.Collections.DataBaseLanguage.FoodPlates.Remove(i);
                        GameData.Core.Collections.DataBaseCore.Foods.Remove(i);
                        GameData.Core.Collections.DataBaseCore.Recipes.Remove(i);
                        GameData.Core.Collections.DataBaseCore.RecipesMapping.Remove(i);
                        GameData.RunTime.Common.RunTimeStorage.Recipes.Remove(i);
                    }
                }                
            }
        }
